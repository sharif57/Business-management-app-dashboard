...
import { useAboutUsGetQuery, useUpdateAboutUsMutation } from "../../redux/features/conditionSlice";
...
const EditTrust = () => {
  const navigate = useNavigate();
  const { data, isLoading, isError, error } = useAboutUsGetQuery();
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await updatePrivacy({ content , pageName: "about-us" }).unwrap();
      toast.success(response?.message || "About Us updated successfully!");
      navigate(-1); // Adjust route as needed
    } catch (err) {
      toast.error(err?.data?.message || "Failed to update about us.");
    }
  };
